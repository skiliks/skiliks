...
class ImportTest extends CDbTestCase
{
    public function test_Full_Import()
    {
        ...
        try {
            ...
            // CommunicationTheme
            $this->assertEquals(4, CommunicationTheme::model()->countByAttributes(['character_id' => null]));
            $this->assertEquals(2, CommunicationTheme::model()->countByAttributes(['phone' => 1]));
            $this->assertEquals(112, CommunicationTheme::model()->countByAttributes(['mail' => 1]));
            $this->assertEquals(49, CommunicationTheme::model()->countByAttributes(['text' => '!проблема с сервером!']));
            $this->assertEquals(8, CommunicationTheme::model()->countByAttributes(['mail_prefix' => 'fwdfwd']), 'fwdfwd');
            $this->assertEquals(8, CommunicationTheme::model()->countByAttributes(['mail_prefix' => 'fwdrere']), 'fwdrere');
            $this->assertEquals(8, CommunicationTheme::model()->countByAttributes(['mail_prefix' => 'fwdrerere']), 'fwdrerere');
            $this->assertEquals(8, CommunicationTheme::model()->countByAttributes(['mail_prefix' => 'rererere']), 'rererere');
