                ...
                if (self::ACTION_OPEN == (string)$log[2] OR self::ACTION_ACTIVATED == (string)$log[2]) {
                    ...
                    if ($log[1] != 13) {
                        ...
                        if (isset($log[4]) && isset($log[4]['mailId'])) {
                            MailBoxService::updateMsCoincidence($log[4]['mailId'], $simId);
                        }
                        continue;
    ...
    public static function setWindowsLog($simId, $logs, $isLastLog = false)
    {
        ...
        foreach ($logs as $log) {
            if (self::ACTION_OPEN == (string)$log[2] || self::ACTION_ACTIVATED == (string)$log[2]) {
                $start_time = gmdate("H:i:s", $log[3]);
                if (LogWindows::model()->count('end_time > :end_time AND sim_id = :sim_id', [
                    'end_time' => $start_time,
                    'sim_id' => $simId
                ])) {
                    throw(new CException('Time overlap. Achtung!'));
                }
                ...
                $log_window->start_time = $start_time;
                $log_window->save();
                continue;
