...
class FixTimeEfficCommand extends CConsoleCommand
{
    public function actionIndex($is_fix = 'false') // 7 days
    {
        ...
        $tmpArray = HeroBehaviour::model()->findAllByAttributes([
            'scenario_id' => 2,
            'code'        => ['214g0', '214g1', '214d5', '214d6', '214d8'],
        ]);
        foreach ($tmpArray as $behaviour) {
...
//                round($managerial_1_value, 0) - 1 != round($data['management'][1]['total'], 0) &&
//                round($managerial_1_value, 0) + 1 != round($data['management'][1]['total'], 0)
