...
class m140326_123336_free_email_services extends CDbMigration
{
	public function up()
	{
        $text = '0-mail.com,0815.ru,0clickemail.com
...
zippymail.info,zoaxe.com';
        ...
        foreach ($list as $domain) {
            $exist = FreeEmailProvider::model()->findByAttributes(['domain' => $domain]);

            if (null == $exist) {
                $freeEmailProvider = new FreeEmailProvider();
                $freeEmailProvider->domain = $domain;
                $freeEmailProvider->security_risk = FreeEmailProvider::FREE_MAIL;
                echo $domain . ($freeEmailProvider->save());
            }
        }
	}
	...
	{
		echo "m140326_123336_free_email_services does not support migration down.\n";
	}
}