...
class MailController extends AjaxController
{
    ...
    public function actionReply()
    {
        ...
        $messageToReply = MailBoxModel::model()->byId($messageId)->find();

        $groupId = (int) $messageToReply->group_id;
        ...
        if ($subject == '') {
            if ($messageToReply->subject_id > 0) {
                $subjectModel = MailThemesModel::model()->byId($messageToReply->subject_id)->find();
                if ($subjectModel) {
        ...
        $result['phrases']['previouseMessage'] = $messageToReply->message;
    ...
    public function actionToPlan()
    {
        try {
            $messageId = (int) Yii::app()->request->getParam('id', false);
            if ($messageId == 0)
                throw new Exception("wrong messageId");

            $sid = Yii::app()->request->getParam('sid', false);
            if (!$sid)
                throw new Exception("wrong sid");

            // определить идентификатор шаблона письма
            $templateId = (int) MailBoxService::getTemplateId($messageId);
            if ($templateId == 0)
                throw new Exception("cant get template for id : $messageId");
    ...
    public function actionAddToPlan()
    {
        try {
            ...
            if (!$model)
                throw new Exception("cant get model by taskId $taskId");
            $name = $model->name;
            if (!empty($messageId)) {
                $message = MailBoxModel::model()->byId($messageId)->find();
                $message->plan = 1;
    ...
    public function actionForward()
    {
        try {
            ...
            $messageToForward = MailBoxModel::model()->byId($mailId)->find();
            if (!$messageToForward)
                throw new Exception("cant get model by id $mailId");
            $subject = $messageToForward->subject;
            $subjectId = $messageToForward->subject_id;
